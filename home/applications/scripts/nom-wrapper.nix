# https://github.com/maralorn/nix-output-monitor/issues/108
{ pkgs, lib, ... }:
...
in
{
  home.shellAliases = nomAliases { inherit (pkgs) nix nixos-rebuild home-manager; };

  # or

  /*
    home.packages = [
      (lib.hiPrio (wrapWithNom {
        inherit (pkgs) nix nixos-rebuild home-manager;
      }))
    ];
  */
}
